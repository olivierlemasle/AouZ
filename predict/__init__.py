...
class Module1:
    ...
    def __str__(self) -> str:
        return "Prediction: {}  Fitness: {}  A: {}  Z: {}".format(self.predict(), self.fitness, self.count_a, self.count_z)
...
class Module2:
    ...
    def update_fitness(self):
        ...
        if (more_successes and new_input_is_success) or (not more_successes and not new_input_is_success):
            if positive_fitness:
                self.fitness *= 4/3
            else:
                self.fitness = 1
        elif (more_successes and not new_input_is_success) or (not more_successes and new_input_is_success):
            if positive_fitness:
                self.fitness = -1
            else:
                self.fitness *= 4/3
    ...
    def __str__(self) -> str:
        return "Prediction: {}  Fitness: {}  Successes: {}  Failures: {}".format(self.predict(), self.fitness, self.count_success, self.count_failure)
